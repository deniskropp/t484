...
class ICoherenceMonitorBridge {
public:
    virtual ~ICoherenceMonitorBridge() = default;
    virtual CoherenceState evaluate(const std::vector<Section> &sections) const = 0;
    virtual void notifyStateChanged(const CoherenceState &state) { (void)state; }
};

class DefaultCoherenceMonitorBridge : public ICoherenceMonitorBridge {
public:
    CoherenceState evaluate(const std::vector<Section> &sections) const override
    {
        CoherenceState s;
...
class CoherenceMonitorBridge {
public:
    static ICoherenceMonitorBridge *current()
    {
        if (!s_customBridge) {
            static DefaultCoherenceMonitorBridge defaultBridge;
            return &defaultBridge;
        }
        return s_customBridge;
    }

    static void setBridge(ICoherenceMonitorBridge *bridge)
    {
        s_customBridge = bridge;
    }

    static void resetToDefault()
    {
        s_customBridge = nullptr;
    }

private:
    inline static ICoherenceMonitorBridge *s_customBridge = nullptr;
};

inline CoherenceState deriveCoherence(const std::vector<Section> &sections)
{
    return CoherenceMonitorBridge::current()->evaluate(sections);
}
