...
| `cmd/exec` | `ocs-node-engine`, `genai`, `nexus-export` (planned) | command payload | **append** |
...
Startup file: `src/assets/seed.ocs`.
...
v0.6 fixture (plan, already parseable): `src/assets/nexus-v0.6.ocs`.
Version attributes live in the `protocol/ocs` qualifier and are opaque text to v0.4.
