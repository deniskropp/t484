...
3. Slash map onto **existing** families only:
   ...
   | `/exec nexus-export` | `cmd/exec:nexus-export` then `exportNexus()` |
   | `/obj [text]` | `data/obj` |
   | `/tas [lines]` | `data/tas` |
...
8. `replyAfter` emits KickForge / KickFlow / KickGuard sections as needed.
   Sets `requestLlm` for natural language, `/exec` (except nexus-export), `/obj`, `/tas`, and non-load sigil pastes.
   Does **not** set `requestLlm` for empty, parse error, document load, `/halt`, `/mode`, `/exec nexus-export`.
