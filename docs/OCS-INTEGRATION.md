...
```
 ...
 t484 implements the **document + gate + display** slice of OCS.
It does not implement RTA descent, swarm joins, or KickLang module compilation.
...
`ocs-mcp-core`, calling `loadText` / `emitText` / halt — not a second parser.
