...
- Raw `⫻` sections → parse. A `protocol/ocs` block **loads** a new document.
...
| Export while gated | `emitText()` is a read — allowed (v0.6 plan; v0.4: copy raw source) |
...
| `src/assets/seed-nexus.ocs` | extra seed |
| `src/assets/nexus-v0.6.ocs` | v0.6 plan fixture (must parse today) |
